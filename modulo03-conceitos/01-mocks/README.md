...
Check se está usando o Node.js LTS (neste momento é o 19)
...
Supondo que você está em `tajs/setup-jest` e já restaurou os pacotes seguindo os passos anteriores execute:
```shell
code ../
...
```shell
 *  Executing task: bash -c 'PROJECT_DIR=$(dirname /Users/seu-nome/cursos/setup-jest/test/template.test.js); cd "$PROJECT_DIR"; npm run test:debug'


> setup-jest@0.0.1 test:debug
